...
export default function ProfileYouScreen() {
  ...
  const { currentUser, backendSyncAttempted, logout, changeUsername } =
    useApp();
  ...
  const [accountEditMode, setAccountEditMode] = useState<
  ...
  useEffect(() => {
    const loadProfile = async () => {
      ...
      try {
        ...
        const loadedDiets = pick(DIET_OPTIONS, user.diets);
        const loadedAvoids = pick(AVOID_OPTIONS, user.avoid_allergens);
        const loadedCuisines = pick(CUISINE_OPTIONS, user.favorite_cuisines);
        setSelectedDiets(loadedDiets);
        setSelectedAvoids(loadedAvoids);
  ...
  const hasPreferenceChanges = useMemo(() => {
    return (
      !arraysEqual(selectedDiets, savedPreferences.diets) ||
      !arraysEqual(selectedAvoids, savedPreferences.avoids) ||
      !arraysEqual(selectedCuisines, savedPreferences.cuisines)
    );
  }, [selectedDiets, selectedAvoids, selectedCuisines, savedPreferences]);
    ...
    <View style={styles.container}>
      ...
      >
          ...
          <View style={styles.buddyHeaderRow}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={styles.userHandle}>
                @{currentUser.username || "pantry.pal"}
              </Text>
        ...
        <View style={styles.sectionBlock}>
              ...
              return (
                ...
                >
                  <Text
                    style={[
                      styles.prefChipText,
                      isSelected && styles.prefChipTextActive,
                    ]}
                  >
                    {diet}
                  </Text>
            ...
            {AVOID_OPTIONS.map((avoid) => {
              ...
              return (
                <Pressable
                  ...
                  style={[
                    styles.prefChip,
                    isSelected && styles.prefChipAvoidActive,
                  ]}
                ...
                >
                  <Text
                    style={[
                      styles.prefChipText,
                      isSelected && styles.prefChipTextAvoidActive,
                    ]}
                  >
                    {avoid}
                  </Text>
          ...
          {/* Save Preferences Button: Only appears when new preferences are selected/changed, or actively saving, or in success state */}
          {(hasPreferenceChanges || isSavingPrefs || prefsSavedSuccess) && (
          ...
          <View style={styles.wasteHeaderRow}>
            ...
            <View style={styles.segmentToggle}>
              <Pressable
                style={[
                  styles.segmentBtn,
                  period === "weeks" && styles.segmentBtnActive,
                ]}
              ...
              <Pressable
                style={[
                  styles.segmentBtn,
                  period === "months" && styles.segmentBtnActive,
                ]}
          ...
          {statsLoading && !stats ? (
            ...
            <>
                  ...
                  {buckets.map((bar, bIdx) => {
                    ...
                    return (
                      <View key={bIdx} style={styles.barColumn}>
                        <View
                          style={[styles.barPillar, { height: totalHeight }]}
                        >
                          <View
                            style={[
                              styles.wastedSegment,
                              { height: wastedHeight },
                            ]}
                          />
                          <View
              ...
              {stats.unpriced_items > 0 && (
                <Text style={styles.wasteTrendText}>
                  {stats.unpriced_items}{" "}
                  {stats.unpriced_items === 1 ? "item has" : "items have"} no
                  price and {stats.unpriced_items === 1 ? "is" : "are"} not
                  counted.
                </Text>
              )}
            </>
      ...
      <AccountEditModal
        ...
        onSavePassword={(current, next) =>
          backendApi.changePassword(userId, current, next)
        }
      ...
      <ChangeBuddyModal
        ...
        onSelectBuddy={(key, name) => {
          ...
          if (backendBuddy && userId) {
            backendApi
              .updateUser(userId, { buddy: backendBuddy })
              .catch((err) => {
                Alert.alert(
                  "Could not save buddy",
