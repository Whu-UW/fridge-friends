...
const BASE_URL = process.env.EXPO_PUBLIC_BACKEND_URL || 'https://fridge-friends-be.fastapicloud.dev';
...
async function main() {
  ...
  await runTest(`GET /users/${activeUserId} (Get Active User Profile)`, async () => {
  ...
  await runTest(`GET /users/${activeUserId}/grocery-items (Fetch User Pantry)`, async () => {
    const items = await request(`/users/${activeUserId}/grocery-items`);
    if (!Array.isArray(items)) throw new Error(`Expected array of grocery items`);
  ...
  await runTest(`POST /users/${activeUserId}/grocery-items (Add New Item)`, async () => {
  ...
  await runTest(`PATCH /grocery-items/{id} (Update Item Quantity & Status)`, async () => {
  ...
  await runTest(`GET /users/${activeUserId}/grocery-items/expiring?include_friends=true (Collaborative Feast Mode Items)`, async () => {
    const expiring = await request(`/users/${activeUserId}/grocery-items/expiring?within_days=7&include_friends=true`);
    if (!Array.isArray(expiring)) throw new Error(`Expected array of expiring items`);
  ...
  await runTest(`GET /users/${activeUserId}/friends (Check Mutual / Pending Friends)`, async () => {
    ...
    const nonFriend = allUsers.find(u => u.id !== activeUserId && !friendIds.includes(u.id));
    if (nonFriend) {
      testFriendId = nonFriend.id;
    }
    const friendList = friends.map(f => `${f.friend?.name} (${f.status})`).join(', ');
  ...
  await runTest(`POST / PATCH / DELETE /users/${activeUserId}/friends (Friend Request Lifecycle)`, async () => {
    // 1. Add friend
    const added = await request(`/users/${activeUserId}/friends`, {
      method: 'POST',
      body: JSON.stringify({ friend_id: testFriendId }),
    });
    if (added.status !== 'pending') throw new Error(`Expected pending status on creation`);

    // 2. Accept friend
    const accepted = await request(`/users/${activeUserId}/friends/${testFriendId}`, {
      method: 'PATCH',
      body: JSON.stringify({ status: 'accepted' }),
    });
    if (accepted.status !== 'accepted') throw new Error(`Expected accepted status`);

    // 3. Remove friend
    await request(`/users/${activeUserId}/friends/${testFriendId}`, { method: 'DELETE' });
